...
static void
file_export_to_svg_dialog_ok_callback (GtkWidget        *w,
				       GtkFileSelection *fs)
{
  char *filename;
  Diagram *dia;
  ...
  diagram_export_to_svg(dia, filename);
...
void
file_export_to_svg_callback(GtkWidget *widget, gpointer data)
{
  DDisplay *ddisp;
  GtkWidget *window = NULL;
  GtkFileSelection *fs;
  ...
  window = gtk_file_selection_new (_("Export to SVG"));
  gtk_window_set_position (GTK_WINDOW (window), GTK_WIN_POS_MOUSE);
  ...
  gtk_signal_connect (GTK_OBJECT (fs->ok_button), "clicked",
		      GTK_SIGNAL_FUNC(file_export_to_svg_dialog_ok_callback),
		      window);
  ...
  /* put a .eps extention on the file name */
  set_default_file_selection_extension (window, ".svg");
  gtk_widget_show (window);
