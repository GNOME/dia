...
static void
object_type_detect_nosheet_callback(gpointer key, gpointer value,
                                    gpointer user_data)
{
  GSList *sheets_list;
  ...
  gchar *sheet_name;
  guint i;
  static gchar *ignore[] =          /* Ignore ObjectType's with these   */
  {                                 /* 'name's.  Mostly used for object */
      "Group",                      /* types that are obsolete and thus */
                                    /* are not found on any sheet.      */
      "Contact - if",
      "Contact - ifnot",
      "Contact - lamp",
      "Contact - relay",
  ...
  for (i = 0; ignore[i]; i++)
    if (!strcmp(((ObjectType *)(value))->name, ignore[i]))
      return;

  /* We also ignore ObjectType's that begin with 'Standard -' */

  sheet_name = object_type_get_sheet_name((ObjectType *)value);
  if (!strcmp(sheet_name, "Standard"))
  {
    g_free(sheet_name);
    return;
  }
  ...
  for (sheets_list = get_sheets_list(); sheets_list && found == FALSE;
       sheets_list = g_slist_next(sheets_list))
  {
    GSList *objects_list = ((Sheet *)(sheets_list)->data)->objects;

    for ( ; objects_list; objects_list = g_slist_next(objects_list))
    {
      if (!strcmp(key, ((SheetObject *)(objects_list)->data)->object_type))
      {
        found = TRUE;
        break;
      }
  ...
  if (found == FALSE)
    object_types_nosheet_list = g_slist_append(object_types_nosheet_list,
                                               g_memdup(value,
                                                        sizeof(ObjectType)));
}

GSList * 
object_types_detect_nosheet(void)
{
  g_slist_foreach(object_types_nosheet_list, (GFunc)g_free, NULL);
  g_slist_free(object_types_nosheet_list);
  g_hash_table_foreach(object_type_table,
                       (GHFunc)object_type_detect_nosheet_callback, NULL);
  return object_types_nosheet_list;
}

gchar *
object_type_get_sheet_name(ObjectType *type)
{
  gchar *pos;
  gchar *sheet_name;

  sheet_name = g_strdup(type->name);
  pos = strchr(sheet_name, '-');
  if (pos)
    return g_strchomp(g_strdelimit(sheet_name, "-", '\0'));
  else
    return sheet_name;
}

gchar *
object_type_strip_sheet_from_name(ObjectType *type)
{
  gchar *pos;

  pos = strchr(type->name, '-');
  if (pos)
    return pos + 2;
  else
    return type->name;
