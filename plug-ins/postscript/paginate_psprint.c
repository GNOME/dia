...
{
  GtkWidget *dialog;
  GtkWidget *vbox, *frame, *grid, *button;
  ...
  gtk_grid_attach (GTK_GRID (grid), iscmd, 0, 0, 1, 1);
  gtk_widget_set_vexpand (iscmd, TRUE);
  gtk_widget_show (iscmd);

  cmd = gtk_entry_new ();
  gtk_grid_attach (GTK_GRID (grid), cmd, 1, 0, 1, 1);
