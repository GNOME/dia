...
class Klass :
	...
	def AddAttributeSql(self, name, type, primary, default, comment, nullable, unique) :
		self.attributes.append ((name, (type, primary, default, comment, nullable, unique)))
		...
		for layer in data.layers :
			...
			for o in layer.objects :
				if o.type.name == "UML - Class" :
					...
					pass
				elif o.type.name == "Database - Table" :
					k = Klass (o.properties["name"].value)
					k.SetComment(o.properties["comment"].value)
					for attr in o.properties["attributes"].value :
						k.AddAttributeSql(attr[0], attr[1], attr[3], attr[6], attr[2], attr[4], attr[5])
					self.klasses[o.properties["name"].value] = k
			...
			for o in layer.objects :
				for c in o.connections:
					for n in c.connected:
						if not n.type.name in ("UML - Generalization", "UML - Realizes", "Database - Reference", "Database - Compound"):
							continue
						if str(n) in edges:
							continue
						edges[str(n)] = None
						...
						if not par.type.name == "UML - Class" and chi.type.name == "UML - Class":
							if not par.type.name == "Database - Table" and chi.type.name == "Database - Table":
								continue
						...
						elif n.type.name == "Database - Reference":
							self.klasses[chi_name].AddReference(par_name)
...
##
# SqlRenderer: export selected UML and Database shapes to Structured Query Language
#
# Added by: Chris Maj
#
# This is different vs. other renderers in this file which only use UML (as of 2019.)
...
#   * UML - Class
#
# Both "Database - Table" and "UML - Class" should produce reasonable CREATE TABLE statements.
# But there's more column-based control in Dia with "Database - Table" eg. check marks for Primary Key.
#
# Note that foreign keys will auto-match on same column names using the
# "Database - Reference" shape, which maps 1->n from primary->foreign.
# This may not always be what you want...
#
# Inheritance was added with PostgreSQL in mind. Please see their LIKE and INHERITS documentation.
# "Database - Compound" and "UML - Realizes" both produce SQL LIKE statements.
# "UML - Generalization" produces SQL INHERIT statements.
#
# Include the word "DEFAULT" in your Default columns for "Database - Table" shapes.
# This is extensible to any extra SQL on a column eg. "DEFAULT 1 CHECK (mycol > 0)" as "Default".
# Similar for "UML - Class" shapes in your Values fields for the Attributes.
#
# TODO:
#   * Make inheritance and foreign key searches recursive without endless looping.
#     Currently only two degrees of multiple inheritance/reference are supported.
#     (This contrasts with other renderers in this file that do not care about ordering.)
#

class SqlRenderer(ObjRenderer) :
	def __init__(self) :
		ObjRenderer.__init__(self)
	def end_render(self) :
		f = open(self.filename, "w")
		...
		d0, d1, d2, d3 = [], [], [], []
		for sk in self.klasses.keys() :
			ptr = self.klasses[sk].parents + self.klasses[sk].templates + self.klasses[sk].references
			if len(ptr) == 0 :
				d1.append(sk)
			else :
				d3.append(sk)
				d2.extend(ptr)
		d0 = sorted(d1)
		for sk in sorted(list(set(d2))) :
			ptr = self.klasses[sk].parents + self.klasses[sk].templates + self.klasses[sk].references
			for px in ptr :
				if d2.count(px) > 0 and d0[-1] != sk :
					d0.append(sk)
		d0.extend(sorted(list(set(d3) - set(d0))))
		for sk in d0 :
			k = self.klasses[sk]
			parents = self.klasses[sk].parents
			templates = self.klasses[sk].templates
			references = self.klasses[sk].references
			f.write ("CREATE TABLE %s\n(\n" % (sk))
			acnt = 0
			for cname, attr in k.attributes :
				acnt += 1
				ctype = attr[0]
				f.write("  ")
				f.write("%s "%(cname) + " "*(16-len(cname)))
				f.write("%s "%(ctype) + " "*(16-len(ctype)))
				# UML - Class attributes is length 5
				# Database - Table attributes is length 6
				if len(attr) == 6 :
					cpkey = attr[1] == 1 and "PRIMARY KEY" or " "*11
					cnotnull = attr[4] == 0 and "NOT NULL" or " "*8
					cunique = attr[5] == 1 and "UNIQUE" or " "*6
					f.write("%s %s %s " % (cpkey, cnotnull, cunique))
				cdefault = attr[2]
				f.write("%s "%(cdefault) + " "*(16-len(cdefault)))
				for r in references :
					rk = self.klasses[r]
					for rcname, rattr in rk.attributes :
						if rcname == cname :
							f.write("REFERENCES %s (%s)" % (r, rcname))
				if acnt < len(k.attributes) :
					f.write(",\n")
				else :
					for t in templates :
						f.write(",\n  LIKE %s INCLUDING ALL" % (templates[0]))
			f.write("\n)\n");
			if len(parents) > 0 :
				f.write("INHERITS (%s)\n" % ','.join(parents))
			f.write(";\n");
			for cname, attr in k.attributes :
				ccomment = attr[3]
				if len(ccomment) > 0 :
					f.write("COMMENT ON COLUMN %s.%s\nIS '%s'\n;\n" % (sk, cname, ccomment));
			if len(k.comment) > 0 :
				f.write("COMMENT ON TABLE %s\nIS '%s'\n;\n" % (sk, k.comment));
			f.write("\n");
		f.close()
		ObjRenderer.end_render(self)
	def draw_line(*args):pass
	def draw_string(*args):pass
	def draw_polygon(*args):pass
	def draw_ellipse(*args):pass
...
dia.register_export ("PyDia Code Generation (SQL)", "sql", SqlRenderer())
