...
static ObjectTypeOps faraday_type_ops =
{
  (CreateFunc) faraday_create,
  (LoadFunc)   faraday_load,
  (SaveFunc)   object_sissi_save,
  (GetDefaultsFunc)   NULL,
  (ApplyDefaultsFunc) NULL,
};

DiaObjectType faraday_type =
{
  "SISSI - faraday",           /* name */
  0,                            /* version */
  (char **) sissi_object_xpm, /* this is the default pixmap */
  &faraday_type_ops      /* ops */
};
...
static ObjectOps object_sissi_ops = {
  ...
  (SetPropsFunc)        faraday_set_props,
  (TextEditFunc) 0,
...
static PropDescription *faraday_describe_props(ObjetSISSI *faraday)
{
...
static DiaObject *faraday_create(Point *startpoint,  void *user_data, Handle **handle1, Handle **handle2)
{
 ...
 element_init(elem, 8, NUM_CONNECTIONS);
  ...
  for (i=0;i<NUM_CONNECTIONS;i++) {
   ...
   object_sissi->border_color=color_red;
  ...
  object_sissi->text = new_text("",action_font, TEXT_FONT_HEIGHT, &pos, &color_black, ALIGN_LEFT);
	...
	object_sissi->nb_others_fixes=7;
...
DiaObject *faraday_load(ObjectNode obj_node, int version, const char *filename)
{
  ObjetSISSI *object_sissi;
  Element *elem;
  DiaObject *obj;
