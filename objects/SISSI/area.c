...
static ObjectTypeOps area_type_ops =
{
  (CreateFunc) area_create,
  (LoadFunc)   area_load,
...
static PropDescription *area_describe_props(ObjetSISSI *area)
{
...
static DiaObject *area_create(Point *startpoint,  void *user_data, Handle **handle1, Handle **handle2)
{
 ...
 element_init(elem, 8, NUM_CONNECTIONS);
  ...
  object_sissi->connections[8].flags = CP_FLAGS_MAIN;
   ...
   object_sissi->fill_colour=color_gris_clear;

  object_sissi->entity=g_strdup("");
/*  declaration of the entity type of area */
...
/* free (filename);end of XML reading */
  ...
  action_font = dia_font_new_from_style(TEXT_FONT,TEXT_FONT_HEIGHT); 
   object_sissi->text = new_text("",action_font, TEXT_FONT_HEIGHT, &pos, &color_black, ALIGN_LEFT);
	...
	object_sissi->nb_others_fixes=5;
  object_sissi_update_data(object_sissi, ANCHOR_MIDDLE, ANCHOR_MIDDLE);
...
DiaObject *area_load(ObjectNode obj_node, int version, const char *filename)
{
  ObjetSISSI *object_sissi;
  Element *elem;
  DiaObject *obj;
